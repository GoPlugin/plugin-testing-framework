...
func TestOverrideCodeEnv(t *testing.T) {
	t.Run("CL env and version", func(t *testing.T) {
		...
		require.Equal(t, "abc", defaultCodeProps["plugin"].(map[string]interface{})["image"].(map[string]interface{})["image"])
		require.Equal(t, "def", defaultCodeProps["plugin"].(map[string]interface{})["image"].(map[string]interface{})["version"])
	})
}
