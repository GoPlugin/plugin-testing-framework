...
func (g *GithubClient) ListLatestCLCoreReleases(count int) ([]*github.RepositoryRelease, error) {
	ctx := context.Background()
	releases, _, err := g.client.Repositories.ListReleases(ctx, "goplugin", "plugin", &github.ListOptions{PerPage: count})
	return releases, err
}
...
func (g *GithubClient) ListLatestCLCoreTags(count int) ([]*github.RepositoryTag, error) {
	ctx := context.Background()
	tags, _, err := g.client.Repositories.ListTags(ctx, "goplugin", "plugin", &github.ListOptions{PerPage: count})
	return tags, err
}
